...
try {
  ...
  $rsa=[Security.Cryptography.RSA]::Create(3072); try {
    ...
    $common=@('/Qp','/DTestMode','/DCROSS_SCHEMA_FULL','/DMinimumDirectVersion=1.0.9',"/DTestAppIdKey=$id","/DTestSuffix=$id","/DTestInstallRoot=$install","/DTestDataRoot=$data","/DTestMutexName=$id","/DUpdatePackage=$zip","/DUpdateManifest=$manifest","/DUpdateSignature=$sig")
