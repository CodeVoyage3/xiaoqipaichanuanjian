...
{
    private const string TestOnlySyntheticTarget = "99.0.0";
    ...
    public async Task RealAppMaintenancePreparerAndExternalUpdaterCommitFixture10()
    {
        ...
        var root = FindRoot(); var oldSource = Path.Combine(root, "src", "StoreExpiryInspector", "bin", "Release", "net10.0-windows", "s9t07test", "net10.0-windows"); var fixture = Path.Combine(root, "tests", "StoreExpiryInspector.S9T07Fixture", "bin", "Release", "net10.0-windows", "test-only-synthetic-target", "fixture-run"); var updaterSource = Path.Combine(root, "src", "StoreExpiryInspector.Updater", "bin", "Release", "net10.0", "win-x64", "s9t05test", "net10.0", "win-x64"); var app = Path.Combine(install, "app"); CopyDirectory(oldSource, app); CopyDirectory(updaterSource, updaterRoot); var oldExe = Path.Combine(app, "StoreExpiryInspector.exe"); var updaterIdentity = Path.Combine(data, "updater-identity.json"); string? operation = null; Process? parent = null; DateTime parentStarted = default;
        try
        {
    ...
    public async Task ExternalUpdaterCompletesSchemaFixtureMigration(int target)
    {
        ...
        Directory.CreateDirectory(app); File.WriteAllText(Path.Combine(app, "source.txt"), "source"); CopyDirectory(Path.Combine(FindRoot(), "tests", "StoreExpiryInspector.S9T07Fixture", "bin", "Release", "net10.0-windows", "test-only-synthetic-target", "fixture-run"), staging);
        ...
        var journal = new UpdateJournal(operation, "StoreExpiryInspector", install, data, app, staging, old, new string('a', 64), "1.0.2", TestOnlySyntheticTarget, parentPid, parentStarted, UpdatePhase.Prepared, TreeFingerprint.Create(app), TreeFingerprint.Create(staging), now, now, Schema: new SchemaUpdateJournal(SchemaPhase.SnapshotVerified, snapshot, source, migrations, token));
    ...
    public async Task RealProductionOldRollbackRestoresSchema9AndLoadsOldShell()
    {
        ...
        var app = Path.Combine(install, "app"); var staging = Path.Combine(install, "app.staging-" + operation); var old = Path.Combine(install, "app.old-" + operation); CopyDirectory(productionPublish, app); CopyDirectory(Path.Combine(FindRoot(), "tests", "StoreExpiryInspector.S9T07Fixture", "bin", "Release", "net10.0-windows", "test-only-synthetic-target", "fixture-run"), staging);
        ...
        Assert.Equal(new Version(99, 0, 0, 0), AssemblyName.GetAssemblyName(Path.Combine(staging, "StoreExpiryInspector.dll")).Version);
        ...
        using var parent = Process.Start(new ProcessStartInfo("cmd.exe", "/c exit 0") { UseShellExecute = false })!; var parentPid = parent.Id; var parentStarted = parent.StartTime.ToUniversalTime(); await parent.WaitForExitAsync();
        var now = DateTimeOffset.UtcNow; var journal = new UpdateJournal(operation, "StoreExpiryInspector", install, data, app, staging, old, new string('a', 64), "1.0.3", TestOnlySyntheticTarget, parentPid, parentStarted, UpdatePhase.Prepared, oldTree, TreeFingerprint.Create(staging), now, now, Schema: new SchemaUpdateJournal(SchemaPhase.SnapshotVerified, snapshot, source, target, token)); var journalPath = Path.Combine(data, "updates", operation, "journal.json"); await File.WriteAllTextAsync(journalPath, JsonSerializer.Serialize(journal));
