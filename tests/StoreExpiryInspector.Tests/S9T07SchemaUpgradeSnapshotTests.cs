...
public sealed class S9T07SchemaUpgradeSnapshotTests
{
    ...
    public async Task RealAppMaintenancePreparerAndExternalUpdaterCommitFixture10()
    {
        ...
        try
        {
            ...
            var zip = Path.Combine(packageRoot, "StoreExpiryInspector-1.0.3-win-x64.zip"); using (var archive = ZipFile.Open(zip, ZipArchiveMode.Create)) { AddTree(archive, fixture, ""); AddTree(archive, updaterSource, "Updater"); }
            var hash = Convert.ToHexString(SHA256.HashData(File.ReadAllBytes(zip))).ToLowerInvariant(); var target = ExpectedMigrations.Concat(["20260905120000_S9T07Fixture10"]).ToArray(); var manifest = System.Text.Encoding.UTF8.GetBytes($"{{\"schemaVersion\":1,\"version\":\"1.0.3\",\"releaseTag\":\"v1.0.3\",\"repository\":\"CodeVoyage3/xiaoqipaichanuanjian\",\"channel\":\"stable\",\"rid\":\"win-x64\",\"minimumProtocolVersion\":2,\"package\":{{\"fileName\":\"{Path.GetFileName(zip)}\",\"bytes\":{new FileInfo(zip).Length},\"sha256\":\"{hash}\"}},\"targetMigrations\":[{string.Join(',', target.Select(x => $"\"{x}\""))}],\"source\":{{\"minVersion\":\"1.0.2\",\"maxVersion\":\"1.0.2\",\"minMigration\":\"{target[0]}\",\"maxMigration\":\"{ExpectedMigrations[^1]}\"}}}}"); using var rsa = RSA.Create(2048); var signature = rsa.SignData(manifest, HashAlgorithmName.SHA256, RSASignaturePadding.Pss); var manifestPath = Path.Combine(packageRoot, "update-manifest.json"); var signaturePath = Path.Combine(packageRoot, "update-manifest.sig"); File.WriteAllBytes(manifestPath, manifest); File.WriteAllBytes(signaturePath, signature);
    ...
    public async Task ExternalUpdaterCompletesSchemaFixtureMigration(int target)
    {
        ...
        var journal = new UpdateJournal(operation, "StoreExpiryInspector", install, data, app, staging, old, new string('a', 64), "1.0.2", "1.0.3", parentPid, parentStarted, UpdatePhase.Prepared, TreeFingerprint.Create(app), TreeFingerprint.Create(staging), now, now, Schema: new SchemaUpdateJournal(SchemaPhase.SnapshotVerified, snapshot, source, migrations, token));
    ...
    public async Task RealProductionOldRollbackRestoresSchema9AndLoadsOldShell()
    {
        ...
        var source = ExpectedMigrations; var target = source.Concat(["20260905120000_S9T07Fixture10"]).ToArray(); var snapshot = SchemaUpgradeSnapshots.Create(data, operation, "1.0.2", source); var oldTree = TreeFingerprint.Create(app); var token = Guid.NewGuid().ToString();
        using var parent = Process.Start(new ProcessStartInfo("cmd.exe", "/c exit 0") { UseShellExecute = false })!; var parentPid = parent.Id; var parentStarted = parent.StartTime.ToUniversalTime(); await parent.WaitForExitAsync();
        var now = DateTimeOffset.UtcNow; var journal = new UpdateJournal(operation, "StoreExpiryInspector", install, data, app, staging, old, new string('a', 64), "1.0.2", "1.0.3", parentPid, parentStarted, UpdatePhase.Prepared, oldTree, TreeFingerprint.Create(staging), now, now, Schema: new SchemaUpdateJournal(SchemaPhase.SnapshotVerified, snapshot, source, target, token)); var journalPath = Path.Combine(data, "updates", operation, "journal.json"); await File.WriteAllTextAsync(journalPath, JsonSerializer.Serialize(journal));
        ...
        try
        {
            ...
            using var ack = JsonDocument.Parse(await File.ReadAllTextAsync(Path.Combine(data, "updates", operation, "health-ack.json"))); Assert.Equal("1.0.2", ack.RootElement.GetProperty("version").GetString()); Assert.Equal(source, ack.RootElement.GetProperty("migrations").EnumerateArray().Select(item => item.GetString()).ToArray()); Assert.True(ack.RootElement.GetProperty("uiLoaded").GetBoolean());
    ...
    public async Task OldPendingGraceIdentityAllowsLegitimateSettingsChangeAfterExactProcessExit()
    {
        ...
        try
        {
            using var process = Process.Start(new ProcessStartInfo(updater) { UseShellExecute = false, ArgumentList = { "--journal", journalPath }, Environment = { ["S9_T07_NORMAL_PROCESS_RECORD"] = record, ["S9_T07_NORMAL_WAIT_MS"] = "3000", ["S9_T07_UPDATER_OBSERVED_IDENTITY_MARKER"] = identified } })!;
