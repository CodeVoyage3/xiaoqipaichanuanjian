...
public sealed class TodayInspectionPlanExportUseCaseTests
{
    ...
    private static long AddTask(StoreDbContext context, string code, string barcode, string name, string category, string? policy, int stock, DateOnly expiry, DateOnly? production = null, string status = "open", ExpiryManagementStatus managementStatus = ExpiryManagementStatus.Managed)
    {
        ...
        context.SaveChanges();
        if (policy is not null && !context.ScopeBaselines.Any(value => value.ScopeKey == category && value.PolicyCode == policy))
        {
