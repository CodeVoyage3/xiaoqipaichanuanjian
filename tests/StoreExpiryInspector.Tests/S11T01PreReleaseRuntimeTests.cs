...
public sealed class S11T01PreReleaseRuntimeTests
{
    ...
    public async Task PreReleaseProductionEquivalentUpgradeAndRuntimeReset()
    {
        ...
        using var ui = Start(exe, data); var main = await MainWindow(ui, TimeSpan.FromSeconds(30)); Assert.True(WindowIconPresent(main), "WPF main-window/taskbar icon missing"); var dataMetrics = Metrics(main); AssertAligned(dataMetrics); var dataTexts = TextNames(main); var importedStateObserved = dataTexts.Contains("数据正常") && !dataTexts.Contains("暂无导入数据") && dataTexts.Any(text => text.StartsWith("最近一次成功导入：", StringComparison.Ordinal)); Assert.True(importedStateObserved, "seeded dashboard did not enter the production imported-data state"); Capture(main, Path.Combine(evidence, "dashboard-data.png"));
        ...
        await WaitForBusinessCount(database, 0, TimeSpan.FromSeconds(20)); await WaitForWindowMissing(ui.Id, "提醒设置", TimeSpan.FromSeconds(10)); Assert.Equal(backupCount + 1, BackupCount(database)); var emptyMetrics = Metrics(main); AssertAligned(emptyMetrics); var resetInitialStateObserved = TextNames(main).Contains("暂无导入数据") && BusinessCount(database) == 0; Assert.True(resetInitialStateObserved); Capture(main, Path.Combine(evidence, "dashboard-empty.png")); Stop(ui);
        using var reopened = Start(exe, data); var reopenedMain = await MainWindow(reopened, TimeSpan.FromSeconds(30)); var resetRestartObserved = TextNames(reopenedMain).Contains("暂无导入数据") && BusinessCount(database) == 0; Assert.True(resetRestartObserved); Stop(reopened);
        SeedAppStateDates(database); var stateBackupCount = BackupCount(database); using var stateUi = Start(exe, data); var stateMain = await MainWindow(stateUi, TimeSpan.FromSeconds(30)); Invoke(Find(stateMain, "设置", ControlType.Button)); settings = await Window(stateUi.Id, "提醒设置", TimeSpan.FromSeconds(10)); Invoke(Find(settings, "重置业务数据", ControlType.Button)); warning = await Window(stateUi.Id, "重置业务数据", TimeSpan.FromSeconds(10)); Invoke(Find(warning, "继续", ControlType.Button)); second = await Window(stateUi.Id, "再次确认重置", TimeSpan.FromSeconds(10)); Invoke(Find(second, "确认重置", ControlType.Button)); done = await Window(stateUi.Id, "重置业务数据", TimeSpan.FromSeconds(30)); Invoke(Find(done, "知道了", ControlType.Button)); Assert.Equal(stateBackupCount + 1, BackupCount(database)); var appStateOnlyResetObserved = AppStateDatesAreNull(database); Assert.True(appStateOnlyResetObserved); Stop(stateUi);
        Reimport(database, data); var reimportObserved = BusinessCount(database) > 0; Assert.True(reimportObserved);

        var result = new { marker = "PRE_RELEASE_PRODUCTION_EQUIVALENT", officialGitHubBytes = false, sourceVersion = "1.0.2", targetVersion = "1.0.3", migrationCount = 9, requestCount = 8, dataPreserved = true, importedStateObserved, runtimeReset = resetInitialStateObserved, resetCancelNoWrite = true, resetFailureAtomic = true, resetRestartEmpty = resetRestartObserved, appStateOnlyReset = appStateOnlyResetObserved, windowAndTaskbarIconPresent = true, reimportViaExistingApplicationUseCase = reimportObserved, dataMetrics, emptyMetrics, screenshots = new[] { "dashboard-data.png", "dashboard-empty.png" } };
    ...
    private static string[] TextNames(AutomationElement root) => root.FindAll(TreeScope.Descendants, new PropertyCondition(AutomationElement.ControlTypeProperty, ControlType.Text)).Cast<AutomationElement>().Select(element => element.Current.Name).ToArray();
