...
public sealed class Stage4ViewModelTests
{
    ...
    public void PendingTaskUiUsesDropdownFiltersAndKeepsStageBadgeContract()
    {
        ...
        var ordinaryButtonStart = app.IndexOf("<Style TargetType=\"Button\">", StringComparison.Ordinal);
        var ordinaryButtonEnd = app.IndexOf("<Style x:Key=\"PrimaryButtonStyle\"", ordinaryButtonStart, StringComparison.Ordinal);
        Assert.True(ordinaryButtonStart >= 0 && ordinaryButtonEnd > ordinaryButtonStart);
        var ordinaryButton = app[ordinaryButtonStart..ordinaryButtonEnd];
        ...
        var pendingStart = window.IndexOf("<!-- 待排查任务 -->", StringComparison.Ordinal);
        var pendingEnd = window.IndexOf("<!-- 排查历史 -->", pendingStart, StringComparison.Ordinal);
        Assert.True(pendingStart >= 0 && pendingEnd > pendingStart);
        var pending = window[pendingStart..pendingEnd];
        Assert.Equal(2, pending.Split("Text=\"{Binding Label}\"").Length - 1);
        Assert.Contains("SelectionBoxItemTemplate", window, StringComparison.Ordinal);
        Assert.DoesNotContain("DisplayMemberPath=\"Label\"", pending, StringComparison.Ordinal);
        Assert.Contains("SelectedValuePath=\"CanonicalStage\"", pending, StringComparison.Ordinal);
        Assert.Contains("SelectedValuePath=\"CategoryName\"", pending, StringComparison.Ordinal);
