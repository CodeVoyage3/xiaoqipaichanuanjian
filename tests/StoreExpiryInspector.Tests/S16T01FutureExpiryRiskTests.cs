...
public sealed class S16T01FutureExpiryRiskTests
{
    ...
    public async Task FailedLoadClearsPriorRowsAndStaticPrototypeContractRemainsPresent()
    {
        ...
        var root = FindRepositoryRoot(); var xaml = File.ReadAllText(Path.Combine(root, "src", "StoreExpiryInspector", "UI", "MainWindow.xaml")); var codeBehind = File.ReadAllText(Path.Combine(root, "src", "StoreExpiryInspector", "UI", "MainWindow.xaml.cs"));
        ...
        var gridStart = xaml.IndexOf("<DataGrid Grid.Row=\"1\"", StringComparison.Ordinal); var gridEnd = xaml.IndexOf("</DataGrid>", gridStart, StringComparison.Ordinal); Assert.True(gridStart >= 0 && gridEnd > gridStart);
        var grid = xaml[gridStart..(gridEnd + "</DataGrid>".Length)]; var headers = new[] { "商品名称", "商品条码", "当前库存", "当前阶段", "后续阶段", "预计进入后续阶段日期", "距离节点" }; var headerOffsets = headers.Select(header => grid.IndexOf($"Header=\"{header}\"", StringComparison.Ordinal)).ToArray();
        Assert.Equal(7, grid.Split("Header=\"", StringSplitOptions.None).Length - 1); Assert.All(headerOffsets, offset => Assert.True(offset >= 0)); Assert.Equal(headerOffsets.OrderBy(offset => offset), headerOffsets); Assert.DoesNotContain("Header=\"商品编码\"", grid, StringComparison.Ordinal); Assert.DoesNotContain("Binding=\"{Binding ProductCode}\"", grid, StringComparison.Ordinal);
        Assert.Contains("FutureRiskDataGrid_PreviewMouseWheel", codeBehind, StringComparison.Ordinal); Assert.Contains("scrollViewer.LineUp()", codeBehind, StringComparison.Ordinal); Assert.Contains("scrollViewer.LineDown()", codeBehind, StringComparison.Ordinal); Assert.Contains("e.Handled = true", codeBehind, StringComparison.Ordinal);
