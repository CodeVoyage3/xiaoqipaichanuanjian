...
public sealed class LifecycleEventDatabaseTests
{
    ...
    private static Product AddProduct(StoreDbContext context, string productCode)
    {
        ...
        {
            var legacyProduct = InsertLegacyProduct(context, productCode);
    ...
    private static Product InsertLegacyProduct(StoreDbContext context, string productCode)
    {
        ...
        try
        {
            ...
            var parameter = command.CreateParameter(); parameter.ParameterName = "$code"; parameter.Value = productCode; command.Parameters.Add(parameter);
            return new Product { Id = Convert.ToInt64(command.ExecuteScalar()), ProductCode = productCode };
        }
