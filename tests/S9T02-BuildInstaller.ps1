...
$version = [Diagnostics.FileVersionInfo]::GetVersionInfo((Join-Path $publish 'StoreExpiryInspector.exe')).FileVersion
if ($version -notmatch '^\d+\.\d+\.\d+\.0$') { throw 'Published executable has an unsupported FileVersion.' }
...
else { $arguments += "/DAppVersion=$($version.Substring(0, $version.Length - 2))" }
