...
│  └─ UI                                   仅有占位主窗口
└─ tests/StoreExpiryInspector.Tests         131 项测试
```
...
## Stage 2 批准方向（S2-T01 至 S2-T06 已完成）
...
3. `确认应用`：S2-T04 已实现确认前文件哈希复核和内存契约；S2-T05 已实现导入前安全快照；S2-T06 已实现单个 SQLite 写事务及原始工作簿保存。最近两份裁剪与撤销仍未实现。Stage 2 不顺带执行 Stage 3 状态机。
...
- 最近两份原始工作簿裁剪、导入撤销及完整备份恢复均尚未实现；当前备份文件能力仅限 S2-T05 导入前安全快照。
