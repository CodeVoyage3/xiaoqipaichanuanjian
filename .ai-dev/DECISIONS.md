...
- 状态：技术栈、业务原则与 Stage 2 八卡拆分已批准；S2-T01 至 S2-T04 已验收通过，导入前快照与正式应用尚未实现。
...
- 状态：数据表与导入前快照文件服务已实现；保留、恢复与工作簿策略尚未实现。
- 决策：正式导入、恢复、版本升级前创建安全快照；自动备份保留最近 7 份；最近 2 份成功导入原始 Excel 以 BLOB 和 SHA-256 保存于 SQLite。
- 当前事实：已有 `backups`、`import_workbooks` 和相关约束；S2-T05 已实现 SQLite 在线快照、哈希、结构验证和原子改名，返回纯内存追溯元数据但不写 `backups`。恢复与裁剪服务仍未实现。
...
- 状态：已批准；S2-T04 已在确认契约中落实，尚未写入导入记录。
