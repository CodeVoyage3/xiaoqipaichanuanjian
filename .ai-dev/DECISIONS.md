...
- 状态：技术栈、业务原则与 Stage 2 八卡拆分已批准；S2-T01 至 S2-T06 已验收通过，工作簿裁剪与撤销尚未实现。
...
- 状态：数据表、导入前快照文件服务与成功导入工作簿保存已实现；保留和恢复策略尚未实现。
- 决策：正式导入、恢复、版本升级前创建安全快照；自动备份保留最近 7 份；最近 2 份成功导入原始 Excel 以 BLOB 和 SHA-256 保存于 SQLite。
- 当前事实：已有 `backups`、`import_workbooks` 和相关约束；S2-T05 已实现 SQLite 在线快照、哈希、结构验证和原子改名，S2-T06 在成功导入事务中保存对应 BackupRecord 与原始工作簿。恢复与最近两份裁剪仍未实现。
