# V1-F01 冷启动模型设计（第一轮规则已确认；Stage policy 待确认）
...
4. 已过期且在已批准动态窗口内：直接进入正常 open task，来源可审计为“首次历史补查”；不能伪造已完成记录。
...
1. 已确认：商品大类 + policy code/version 范围级基线、`5% clamp(3,60)`、窗口内直接 open task、窗口外仅历史基线、到期当天 expired、原表临期折扣标识不作强制准入。
2. 仍阻断：源表只有语义中性的“折扣日期”，没有独立 5 折、2 折、收仓 / 下架三节点字段；产品经理或数据提供方必须补充每个管理范围的完整三节点定义与 policy/version 映射。
