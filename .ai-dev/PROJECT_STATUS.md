...
- 项目：门店效期排查软件 V1
- 当前阶段：Stage 7 已获批并实施 S7-T01；等待独立验收与正式数据哈希门禁
- 状态：S7-T01 实现及仓库技术门禁通过，尚未正式归档；未进入 S7-T02
- 当前分支：`master`
...
- S7-T01 已复用 Stage 2 SQLite 在线快照能力，新增本地数据库备份 Application 契约、独立备份目录、JSON/既有 `BackupRecord` 元数据、完整性/migration/SHA-256 核验和单次并发门禁；专项 6/6、共享快照回归 12/12、Stage 6 52/52、Stage 5 51/51、Stage 4 179/179、Stage 3 170/170、Release 641/641、build 0/0、EF 无漂移、migration=8、依赖未变。正式库本轮只读大小 299008 bytes、进程 0，但 SHA-256 仍受异常 Junction 阻断；等待独立验收后归档。
...
- S7-T01 技术验收与待办门禁：`.ai-dev/ACCEPTANCE/S7-T01.md`
...
- S7-T01 已获本轮用户授权并完成生产实现与仓库技术门禁，但因独立 Sol 复核和正式库本轮 SHA-256 回执未闭环，暂不正式归档。不得创建、编号、派发或实施 S7-T02。
