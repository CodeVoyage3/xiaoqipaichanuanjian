...
- 项目：门店效期排查软件 V1
- 当前阶段：Stage 4｜S4-T07 已独立验收通过，停止等待下一步批准
- 状态：Stage 0～Stage 3 整体通过；S4-T01～S4-T07 已独立验收通过；未创建 S4-T08/S4-T09
- 当前分支：`master`
...
- 当前 Release 全量 498/498；S4-T01～S4-T07 精确回归 150/150；Stage 3 精确证据 170/170；Release build 0 警告/0 错误；EF 无漂移；migration 仍为 8 条。
...
- 当前 WPF 主窗已具备 Shell、首页、待排查列表、数据导入、排查详情、草稿、逐项重新确认与库存修正；正式提交、超库存确认及完成/陈旧状态 UI 仍不存在，不得提前接入或把业务规则放进 ViewModel/code-behind。
...
- S4-T07 验收见 `.ai-dev/ACCEPTANCE/S4-T07.md`；本卡通过后按用户要求立即停止，不自动创建 S4-T08/S4-T09。
