...
全新 Terra 实施，Sol 独立验收；GUI PASS 前不 CLOSED/ACCEPTED。
FULL=NOT_RUN / NO_FULL；Version1.1.3；migration10；migration11 NOT_CREATED；原dirty main不触碰。
详见 STAGES/STAGE-25.md、TASKS/S25-T01.md、ACCEPTANCE/S25-T01.md（HANDOFF路径以 .ai-dev 为根）。
...
GitHub 公网匿名 fresh 下载四资产与冻结候选逐项全等：Setup `StoreExpiryInspector-Setup-1.1.1.exe`=`75356358` bytes / SHA256 `5CC4A3C057C9DAC3C436CAB44BA999F2115A2A7229EE3BF09E9C5449DF63BFCB`；ZIP `StoreExpiryInspector-1.1.1-win-x64.zip`=`109462493` / `A75826D1354A1A0CE467AFC09E80601ABF8A6E7CC2C1C57CDCA559D9344A1732`；manifest=`914` / `8E61834C998D4E482CC9EFF4E16CA8E3944EF0D26F19D35EF8C962D36AEF9904`；signature=`384` / `A4AF74B966892F84DFFF54FE3134E0019E107F2683BC45F6997E8953AE378324`。production RSA-PSS/SHA256 trust fingerprint=`565956021399C88A8B13DD0873D2A801F6675EAB44BEB4FC8EBE53C71FEFBADC`，production verifier=`PASS`；公网字节等价保持该验证结论。

App/Updater=`1.1.1 / 1.1.1.0`；migrationCount=`10`，migration11=`NOT_CREATED`；setupCompatibility=`SAME_SCHEMA_SLIM`，minimumDirectVersion=`1.1.0`，crossSchemaAllowed=`false`。Stage22=`CLOSED / ACCEPTED`；TEMP/GUID App 启动与 Stage22 最小页面 smoke=`PASS`；Builder/Release Contract/signing/manifest/Slim/Updater package targeted=`45/45 PASS`，数据库验证专项=`1/1 PASS`；Release build=`PASS`。

A/B/D/E=`EVIDENCE_REUSED / UNCHANGED_PRODUCTION_PATH`；C=`EXISTING_REAL_E2E_EVIDENCE_REUSED + CURRENT_CANDIDATE_PACKAGE_VERIFICATION`。`FULL=NOT_RUN / NO_FULL`；正式数据库=`NO ACCESS`；未重跑 production Setup、A/B/C/D/E、Sandbox 或 FULL。

夸克正式目录已回读同一 Setup 为 `75356358` bytes，永久公开分享=<https://pan.quark.cn/s/eefc5b1406ea>，客态分享内层验证=`PASS`。Gitee `master/latest.json` commit=`86667613f9eadb4a256bdb46199b12379de321b3`，固定 Raw 匿名 HTTP `200`，严格保持 `version/releaseNotes/manualDownloadUrl` 三字段，`version=1.1.1` 且 URL 指向该夸克分享。GitHub 继续作为唯一自动更新与发行身份权威。

正式 dirty 工作树保持原 `1 modified + 4 untracked`；本发布轮未修改生产代码、Schema、migration、Installer、Updater 协议、trust anchor 或冻结资产。v1.1.1=`RELEASED / ACCEPTED`，Release governance=`CLOSED`。

# 2026-09-14：Stage22 CLOSED / ACCEPTED

fresh fetch 收口基线=`origin/main@a6a47f2a255f3eaad5687a7700d72818d0ee5898`；final S22-T02 implementation=`ba02834c03dec5b40a3d0886771afc7fddc0d837`，确认包含 `8ccc5e84b5aad8c3eae740c764506ef502662e7c` 与 GUI R1。S22-T01=`CLOSED / ACCEPTED`；S22-T02=`CLOSED / ACCEPTED / TECHNICAL_PASS / USER_GUI_PASS`；S22-T03=`NOT_CREATED`；Stage22=`CLOSED / ACCEPTED`。
...
migrationCount=`10`；migration11=`NOT_CREATED`；Schema/Version/Installer/Updater/Release diff=`0`；正式版本保持 `v1.1.0`；`FULL=NOT_RUN / NO_FULL`。用户最终真实 WPF GUI=`PASS`；返修定向=`32/32 PASS`、Release App build=`0 warning / 0 error`，收口无生产改动且未重跑。Stage22 已具备进入下一阶段或独立 Release 治理的条件，但本轮均未启动。
...
- S22-T02=`NOT_CREATED / NOT_STARTED / NEXT`；不得在当前话题创建或实施；不得创建 S22-T03。
...
- final implementation=`9d5ab1872f1f12795bce2d3617e9fbe80acb70ce`；accepted pre-closeout candidate=`4cb8ec526c0e0271088818e4772d9313ef9c422b`；Terra / medium / clean worktree 实施，Sol 独立技术验收=`PASS`，用户最终真实 WPF GUI=`PASS`。
- S22 专项=`1/1 PASS`；导航聚焦回归=`2/2 PASS`；Release App build=`PASS / 0 error`（仅 `NU1900`）；库存修正专项=`24/24 PASS`；禁止范围 diff=`0`；正式 dirty 工作区未变化。
...
- 聚焦返修 final=`803ffb985330886164391c0ea0aa5d9236d5888e`：商品明细删除“最近导入”列并扩宽关键字段；详情身份区改为商品编码/商品条码/大类/最近导入四栏。Release App build=`PASS / 0 error`，S18-T03=`2/2 PASS`，`FULL=NOT_RUN`。
- R7 final=`6713578e6dce4ec6c2524212e974aaaf5e97e177`：删除商品详情底部“最近导入信息”卡片及排查详情普通固定底栏重复“修正库存”，顶部入口和既有库存修正链保持。库存修正专项=`24/24 PASS`；顶部入口=`PASS / NO MUTATION`。
- R8 final implementation=`9d5ab1872f1f12795bce2d3617e9fbe80acb70ce`：商品明细独立 Footer 使用顶部轻分隔、12 DIP 上留白、22 DIP 底部留白，页码与按钮垂直居中；1024×600 DIP / 150% 实窗完整可见。用户最终 GUI=`PASS`，S22-T01 已正式 `CLOSED / ACCEPTED`。
...
- S20-T01=`CLOSED / ACCEPTED`，不得重新打开；S20-T02=`CLOSED / ACCEPTED`；Stage20=`CLOSED / ACCEPTED`，不创建 S20-T03。
- Stage20 最终交付两项能力：Core Release Builder 以 `Version + CandidateSha` 生成本地候选并停止于 `RELEASE_CANDIDATE_READY / PUBLISH_NOT_AUTHORIZED`；Change Impact 判定职责变化、required evidence、历史 frozen evidence 复用资格，并对 UNKNOWN fail closed。
- Stage20 不负责自动执行测试、GUI、FULL、rollback 或发布。
...
- implementation=`7cfc67ec3266b2431f4b13faf4edb7ed4b1662d8`；Sol 最终技术验收=`PASS`。历史样本九类全部 REQUIRED 是该版本真实变化结果，不是未来候选固定要求。
...
- App/Updater=`1.1.0 / 1.1.0.0`；migration=`10`。夸克正式分享=<https://pan.quark.cn/s/a71f5cbeefce>；Gitee Raw `latest.json` fresh HTTP `200`，保持三字段协议且 `version=1.1.0`、`manualDownloadUrl` 指向该正式分享。
- Stage19 继续 `CLOSED / ACCEPTED`。`FAULT_INJECTION_ROLLBACK=NOT_FULLY_VERIFIED / PRODUCT_RISK_ACCEPTED`；已证明的程序树、migration9 数据库、数据库 SHA/业务指纹及 integrity/FK 恢复事实保留，不将完整旧版 ACK 记为 PASS。`FULL=NOT_RUN / NO_FULL`。
- 当前状态=`V1.1.0_RELEASED / PUBLIC_REMOTE_VERIFIED`。下一阶段规划为 `Stage20｜Release Builder`，但 `Stage20=NOT_STARTED`；本收口仅更新治理文档，未修改产品、测试、版本、migration、Installer 或任何远端发布状态，未运行 build/test/publish。
...
- S19+S17+S15=`44/44 PASS`；final Release App build=`0 warning / 0 error`；`FULL=NOT_RUN / NO_FULL`。旧 S9/S14 七项在治理基线同错，未为凑数修改。
- GUI candidate 技术就绪并绑定 `db36d34` 生产 DLL；Sol smoke exit 0、marker PASS、进程0、TEMP残留0。正式数据库=`NO ACCESS`，正式 dirty 仍为原 `1 modified + 4 untracked`。
- Stage19=`IN_PROGRESS / S19-T01_CLOSED / S19-T02_TECHNICAL_ACCEPTANCE_READY / WAITING_USER_GUI`；S19-T02=`IMPLEMENTED / TECHNICAL_ACCEPTANCE_READY / NOT_ACCEPTED`。不得 push main、关闭或发布。

# 2026-09-12 historical：S19-T02 GOVERNANCE_FROZEN / IMPLEMENTATION_DISPATCH_AUTHORIZED
...
- ZIP=`109433662` / `71923637759B9E90D41C4293F01CAE8329F69DE77B50E6DB699D8C832E3A6D55`；Setup=`75331419` / `9979AC7CD6B48FB6DF74CF218A8B7F7926FF315402DD738EA19CD84ADCE3BBEF`；manifest=`869` / `95E909DE059EC0D3F70430FC3145ECCF8C5806AF73966EB47D20D0F4A0093C97`；signature=`384` / `B71713E86756832B080278D1207BFE24E5E784F9F8D431D35019305E2CB6CE04`。
- 夸克正式分享=<https://pan.quark.cn/s/be348f33499a>；Gitee metadata commit=`dd8afd8a9d6c1651f6ec117aef6d80d50097781d`，固定 Raw 匿名 HTTP `200`、三字段精确一致。
- v1.0.9=`RELEASED`；S18-T01/T02/T03=`GUI_ACCEPTED / CLOSED`；Stage18=`CLOSED`。Schema/migration=`NO CHANGE / 9`；正式数据库=`NO ACCESS`；`FULL=NOT_RUN / NO_FULL`；正式 dirty 工作区保持未触碰。


# 2026-09-11 historical：S18-T03 GUI_ACCEPTED / Stage18 CLOSED
...
- BUSINESS LOGIC DIFF=`0`；Schema/migration=`NO CHANGE / 9`；正式数据库=`NO ACCESS`；FULL=`NOT_RUN / NO_FULL`。
- S18-T02=`PLANNED / NOT_DISPATCHED`，仅在新的 Codex 话题由用户另行启动。

# 2026-09-10 historical：S18-T01-R5 WAITING_USER_GUI_ACCEPTANCE

- 唯一批准提醒弹窗视觉基线：`D:\下载\ChatGPT Image 2026年9月10日 21_43_57.png`，SHA256 `AFC6A06E691F3755CD7B4868F00E39308753E08C9EEC072DC98F089EFC182B7B`。
- S18-T01=`IMPLEMENTED / TECHNICAL_ACCEPTANCE_READY / WAITING_USER_GUI_ACCEPTANCE`；R5 从治理后 fresh `origin/main` 由全新 Terra（medium）实施，Sol 独立真实 GUI 门禁。
- BUSINESS LOGIC DIFF=`0`；migration=`9`；FULL=`NOT_RUN`；正式数据库不访问。
- S18-T02=`PLANNED / NOT_DISPATCHED`，不得提前启动。

# 2026-09-10 historical：S18-T01-R1 GUI_VISUAL_REWORK_REQUIRED
...
- Terra `2fce2c999c06302622fd3b77623109c7e0ecd5ba` 已从 fresh `origin/main@9747588fbf7856a1da784e73250c5f4c3058452a` 无冲突集成为 `46890b83e15b5a971a6dc6e8418c85de6b841d5a`；生产实现逐文件等价。
- tests：`INHERITED 6/6 PASS / NOT_RERUN`；Release build：`INHERITED PASS (0 error) / NOT_RERUN`；`NU1900 ×3 = NON_BLOCKER`；Full：`NOT_RUN / NO_FULL`。
- GUI 最终验收尚未执行。保留 `origin/codex/s15-t01-terra`；不 tag、不 Release、不发布 v1.0.6、不创建 S15-T02。
...
- Stage15 = `IN_PROGRESS / GOVERNANCE_FROZEN / IMPLEMENTATION_AUTHORIZED`；S15-T01 = `GOVERNANCE_FROZEN / IMPLEMENTATION_AUTHORIZED / NOT_IMPLEMENTED`。
- 唯一范围：发现新版初始界面精简为版本与“稍后提醒 / 立即更新”；点击后只显示“下载中 / 更新中 / 安装中”；下载仅百分比；取消只复用安全下载阶段；稍后提醒二次告知保持原样。
- 禁止导航/业务 UI、更新底层/协议、SQLite/migration、托盘/提醒扩围；不访问正式数据。默认 `NO_FULL`，只跑最小专项和必要 Release build。
- 治理普通 push 同频后，从该基线创建全新隔离 worktree 和全新 Terra；完成实现与门禁后停止。不 tag、不 Release、不发布 v1.0.6、不创建 S15-T02。


# 2026-09-09 current：S14-T01 / Stage14 已关闭

- 用户正式 v1.0.4 -> 新 v1.0.5 真实在线升级验收通过：在线升级、安装后自动启动、主界面、系统托盘及原数据均正常。`REAL_V104_TO_NEW_V105_ONLINE_UPDATE_VERIFIED = PASS`。
- S14-T01 = `V105_RELEASED / REAL_V104_TO_V105_ONLINE_UPDATE_VERIFIED / CLOSED`；Stage14 = `CLOSED`。
- `PUBLIC_V105_RELEASE_ASSET_EQUIVALENCE = PASS`；`NO_FULL`；`163/164` 唯一缺项仍为 `S9_T07_REAL_OLD_PUBLISH = NOT_RUN / TEST_FIXTURE_UNAVAILABLE / NON_BLOCKER`。
- 本轮仅更新治理文档；未修改生产代码，未重跑测试/build/Full，未访问正式 SQLite，未创建 S14-T02 或 Stage15。


# 2026-09-09 current：新版 v1.0.5 已发布，等待真实在线升级

- Stage14 = `IN_PROGRESS`；S14-T01 = `V105_RELEASED / WAITING_USER_REAL_V104_TO_V105_UPGRADE / NOT_ACCEPTED`。
- GitHub stable/latest `v1.0.5`：Release ID `385203812`，`draft=false`、`prerelease=false`；tag object `118d180882f29c9a8a0c4539e024a401b4a6c312` 解引用到产品 source `cddd897d72c4e95c6fd974d5ea44bc24d9c985c7`。
- `PUBLIC_V105_RELEASE_ASSET_EQUIVALENCE = PASS`：发布后匿名 fresh 下载四项资产，SHA256/size 与冻结候选全等。ZIP `543032FC76947772B4CAD3CC2416D611EC7C78A0D2C82C4993C13428E49D1673` / 109411185；Setup `55680AFCD55D8603E9E3443D23AFCD946DA3FE534BD81A47C1A84D5F6C8DB843` / 75328700；manifest `996C366D938E019984C7FDD569B2FAEE72C01A95CE5DFDD5D2397816B06C5FC0` / 869；signature `05C3A65FF4E8323D5F49857A7612DAF5AC644A93FA9397FFD54D64847BAB8076` / 384。
- 未重建、重签或修改候选 bytes；未运行测试/build/Full，未访问正式 SQLite；`NO_FULL`。
- `v1.0.4 -> v1.0.5 ONLINE UPDATE VERIFIED` 尚未成立。下一步只等待用户使用正式 v1.0.4 执行真实在线升级；未创建 S14-T02。
...
- stable v1.0.5 Release：<https://github.com/CodeVoyage3/xiaoqipaichanuanjian/releases/tag/v1.0.5>；Release ID `384564898`，latest、非 draft、非 prerelease。tag 解引用到产品 source `3b483ef772d6442febfbcdd16ab87c6965839445`，与后续 governance HEAD 严格区分。
- `PUBLIC_V105_RELEASE_ASSET_EQUIVALENCE = PASS`：四项匿名公开下载 bytes/SHA256 与冻结候选全等；production RSA-PSS/SHA256、公钥指纹、manifest v1.0.4-only/migration 9、ZIP 618 项/tree hash/禁入项、App/Updater 1.0.5.0 与 Updater subsystem 2 均通过。v1.0.4 Release/资产不变。
- 本轮仅发布和治理记录，没有生产代码修改；Full/90/7/178/Stage9 矩阵均未运行，NO_FULL 保持。下一步只等待用户正式 v1.0.4 -> v1.0.5 六项真实 GUI/升级回执；通过前 S13-T01 与 Stage13 不关闭。

# 2026-09-08 历史：v1.0.5 最终候选已获技术接受，等待发布授权
...
- 独立 win-x64 self-contained Updater、持久 journal、同卷 staging/app.old 完整树、精确 operation 路径绑定、父 PID/start 身份、mutex、候选只读健康 ACK、正常切换、失败回滚和 manual recovery 已完成。WPF 在候选 ACK 前跳过 Initialize/Migrate/补算/reminder 写入。
- Sol 新鲜 TEMP/GUID migration9：实际 self-contained WPF smoke/verification exit0；preparer 3/3、成功硬杀 27/27、回滚硬杀 9/9、补充失败矩阵 8/8、live-parent 与双 updater 通过。DB SHA 前后不变，无 WAL/SHM，锁/AccessDenied/junction 无混树。
- 最终无过滤 Release 1040/1040、0 failure/skip、10m56s；build 0 warning/error；EF 无漂移，migration --no-connect 9 条且末条固定；diff/secret/禁止项检查通过。完整证据见 `ACCEPTANCE/S9-T05.md` 及两个 S9-T05 结果 JSON。
- 未访问正式安装/数据/数据库/备份；未创建 GitHub Release/tag/asset，未配置生产 trust anchor/private key，未发布真实更新包，未执行 migration。开发机合成通过不代表门店投产。
- 下一卡仅建议 S9-T06 首次签名正式 Release 与干净 Win10/11 同 Schema 1.0.0→1.0.1 端到端升级验收；版本、生产密钥保管、Release/tag/assets 和正式发布均须新授权，不含 Schema/migration、正式业务数据、重置或 Undo。
