...
Stage19 = `IN_PROGRESS / GOVERNANCE_FROZEN / S19_T01_IMPLEMENTATION_DISPATCH_AUTHORIZED`
...
## 技术判断：NO SCHEMA CHANGE

现有数据库合同足以安全实现：`products.product_code` 为唯一索引；`batches` 对有生产日期的 `(product_id, production_date, expiry_date)` 与无生产日期的 `(product_id, expiry_date)` 分别有过滤唯一索引；每商品仅一个 open task；每 task 仅一个 Inspection。不存在需要用户裁决的真实身份歧义。
