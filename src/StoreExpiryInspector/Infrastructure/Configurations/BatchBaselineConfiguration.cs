...
public sealed class BatchBaselineConfiguration : IEntityTypeConfiguration<BatchBaseline>
{
    public void Configure(EntityTypeBuilder<BatchBaseline> entity)
    {
        entity.ToTable("batch_baselines", table =>
        {
            ...
            table.HasCheckConstraint("CK_batch_baselines_catchup_window", "(cold_start_disposition = 'expired_catchup_task' AND catchup_window_days BETWEEN 1 AND 30) OR (cold_start_disposition <> 'expired_catchup_task' AND catchup_window_days IS NULL)");
