...
public sealed class InspectionResultImportSessionViewModel : ViewModelBase
{
    ...
    private string _status = string.Empty, _inspector = string.Empty, _inspectorError = string.Empty, _dateError = string.Empty, _filter = "全部";
    ...
    public DateTime? CheckDateValue { get => _checkDate; set { if (_checkDate == value) return; _checkDate = value; InvalidateDraft(); OnPropertyChanged(); Validate(); } }
    ...
    {
        if (_result is null) { Block("请先读取排查结果文件。", "请先选择并读取已填写的排查计划。"); return; }
        if (_draft is null) await SaveDraftAsync();
        if (_draft is null || CompleteTaskIds.Count == 0) { if (IsFormValid) Block("没有可提交的有效排查结果。", "请填写至少一项排查数量后再提交。"); return; }
        if (CheckDateValue is not DateTime date) return;
        var expired = ExpiredWarning(); if (expired is not null && _confirmExpired?.Invoke(expired) != true) { StatusText = "请复核过期商品库存后再提交。"; return; }
        if (expired is null && _confirmSubmission?.Invoke() != true) { StatusText = "已取消提交数据。"; return; }
        _intent ??= new(CompleteTaskIds, InspectorName, DateOnly.FromDateTime(date), _today(), RequireUtc()); IsActionBusy = true;
        try
        {
            while (true)
            {
                ...
                if (result.Outcome is BulkInspectionSubmissionOutcome.RequiresOverStockConfirmation or BulkInspectionSubmissionOutcome.OverStockConfirmationStale)
                {
                    ...
                    _intent = null; _overStock = []; return;
                }
                ...
                await _refresh(intent.TaskIds); Reset(); StatusText = "提交已成功，相关页面已刷新。"; return;
            }
        }
    ...
    private void Changed() { foreach (var name in new[] { nameof(HasPreview), nameof(CanSaveDraft), nameof(IsFormValid), nameof(CompleteTaskIds), nameof(DraftStatusText), nameof(PreviewSummaryText), nameof(PreviewIssueText), nameof(HasPreviewIssues), nameof(PreviewDetailText), nameof(VisiblePreviewRows), nameof(AllPreviewFilterText), nameof(SubmittablePreviewFilterText), nameof(BlankPreviewFilterText), nameof(StalePreviewFilterText), nameof(InvalidPreviewFilterText) }) OnPropertyChanged(name); SaveDraftCommand.RaiseCanExecuteChanged(); SubmitCommand.RaiseCanExecuteChanged(); }
