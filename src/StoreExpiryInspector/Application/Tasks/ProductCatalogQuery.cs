...
public sealed class ProductCatalogQuery
{
    public ProductCatalogPage Search(StoreDbContext context, ProductCatalogRequest request)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Stage)) query = query.Where(product => product.Batches.Any(batch => batch.TrackingStatus == "active" && batch.CurrentStage == request.Stage));
        ...
        }).OrderByDescending(row => row.HighestPriority).ThenBy(row => row.NearestExpiry == null).ThenBy(row => row.NearestExpiry).ThenBy(row => row.Id).Skip((request.Page - 1) * request.PageSize).Take(request.PageSize).ToArray()
