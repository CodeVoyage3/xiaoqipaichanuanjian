...
public sealed class InspectionPlanDraftApplyUseCase
{
    ...
    private static IReadOnlyList<InspectionPlanRow> Resolve(StoreDbContext context, IReadOnlyList<InspectionPlanRow> rows)
    {
        ...
        foreach (var source in rows)
        {
            ...
            if (item is null || product.EffectiveStockQty <= 0 || product.IsStockZeroTerminated || product.ExpiryManagementStatus != ExpiryManagementStatus.Managed || batch.TrackingStatus != "active" || batch.CurrentStage != item.candidate.Stage || batch.AttentionVersion != item.candidate.AttentionVersion || item.candidate.RequiresReconfirmation) { errors.Add("该行对应批次状态已经变化，本次已跳过，请重新导出最新计划。"); result.Add(source with { Errors = errors }); continue; }
