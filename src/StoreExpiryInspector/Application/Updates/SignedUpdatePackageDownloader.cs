...
public sealed class SignedUpdatePackageDownloader
{
    ...
    private async Task<(UpdatePackageOutcome Outcome, byte[]? Hash)> DownloadAsync(Uri uri, string target, long expected, Action<UpdatePackageProgress>? progress, CancellationToken cancellationToken)
    {
        ...
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { _diagnostics?.Add("request-cancelled", new { stage = "Package", initialHost = uri.IdnHost, initialPathCategory = "release-download", source = "caller" }); return (UpdatePackageOutcome.Cancelled, null); }
        catch (OperationCanceledException) { _diagnostics?.Add("request-cancelled", new { stage = "Package", initialHost = uri.IdnHost, initialPathCategory = "release-download", source = "timeout" }); return (UpdatePackageOutcome.NetworkUnavailable, null); }
        catch (Exception error) { _diagnostics?.Add("body-error", new { stage = "Package", initialHost = uri.IdnHost, initialPathCategory = "release-download", error = _diagnostics.SafeError(error) }); throw; }
    }
    ...
    private async Task<byte[]> ReadSmallAsync(string stage, Uri uri, int limit, UpdatePackageOutcome missing, CancellationToken cancellationToken)
    {
        ...
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested) { _diagnostics?.Add("request-cancelled", new { stage, initialHost = uri.IdnHost, initialPathCategory = "release-download", source = "caller" }); throw; }
        catch (OperationCanceledException) { _diagnostics?.Add("request-cancelled", new { stage, initialHost = uri.IdnHost, initialPathCategory = "release-download", source = "timeout" }); throw new UpdatePackageException(UpdatePackageOutcome.NetworkUnavailable); }
        catch (Exception error) { _diagnostics?.Add("body-error", new { stage, initialHost = uri.IdnHost, initialPathCategory = "release-download", error = _diagnostics.SafeError(error) }); throw; }
    }
