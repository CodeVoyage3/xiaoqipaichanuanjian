...
public static class NormalLaunchHandshake
{
    ...
    public static bool IsBoundSameSchemaIntent(string root, string operationId, string executableRoot)
    {
        var intent = Read(root, operationId);
        return intent.Role == NormalLaunchRole.Candidate && ValidateJournalIntent(root, intent, executableRoot) is null;
    }
    private static SchemaUpdateJournal? ValidateJournalIntent(string root, NormalLaunchIntent intent, string executableRoot)
    {
        ...
        var fields = new[] { "OperationId", "ProductId", "InstallRoot", "DataRoot", "AppPath", "StagingPath", "OldPath", "PackageSha256", "SourceVersion", "TargetVersion", "ParentPid", "ParentStartedUtc", "Phase", "OldTree", "CandidateTree", "CreatedUtc", "UpdatedUtc", "CandidatePid", "CandidateStartedUtc", "LastError", "Schema" };
        UpdateProtocolJson.RequireObject(journal, fields); UpdateProtocolJson.RequireObject(journal.GetProperty("OldTree"), "Files", "Hash"); UpdateProtocolJson.RequireObject(journal.GetProperty("CandidateTree"), "Files", "Hash");
        ...
        var phase = journal.GetProperty("Phase"); var schema = journal.GetProperty("Schema");
        var commonValid = journal.GetProperty("OperationId").GetString() == intent.OperationId
            && string.Equals(NormalizePath(journal.GetProperty("DataRoot").GetString()!), NormalizePath(root), StringComparison.OrdinalIgnoreCase)
            && string.Equals(NormalizePath(journal.GetProperty("AppPath").GetString()!), NormalizePath(executableRoot), StringComparison.OrdinalIgnoreCase)
            && journal.GetProperty(tree).GetProperty("Hash").GetString() == intent.ExpectedTreeHash;
        if (schema.ValueKind == JsonValueKind.Null)
        {
            var committedOrCompleted = IsExactPhase(phase, 9, "Committed") || IsExactPhase(phase, 10, "Completed");
            if (intent.Role != NormalLaunchRole.Candidate || intent.ExpectedOuterPhase != outer || intent.ExpectedSchemaPhase != -1 || !committedOrCompleted || !commonValid) throw new InvalidDataException("普通启动事务状态无效。");
            return null;
        }
        RequireExactPhase(phase, outer, outerName); UpdateProtocolJson.RequireObject(schema, "Phase", "Snapshot", "SourceMigrations", "TargetMigrations", "LaunchToken", "CandidatePid", "CandidateStartedUtc", "LastError"); RequireExactPhase(schema.GetProperty("Phase"), schemaPhase, schemaName);
        if (intent.ExpectedOuterPhase != outer || intent.ExpectedSchemaPhase != schemaPhase || !commonValid || schema.GetProperty("LaunchToken").GetString() != intent.LaunchToken) throw new InvalidDataException("普通启动事务状态无效。");
    ...
    private static bool IsExactPhase(JsonElement element, int number, string name) =>
        (element.ValueKind == JsonValueKind.Number && element.TryGetInt32(out var value) && value == number) || (element.ValueKind == JsonValueKind.String && element.GetString() == name);
    private static void RequireExactPhase(JsonElement element, int number, string name)
    {
        if (!IsExactPhase(element, number, name)) throw new InvalidDataException("普通启动事务状态无效。");
    }
